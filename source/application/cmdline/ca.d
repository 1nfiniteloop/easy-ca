...
class CertificateAuthorityCmdlinePlugin : Application
{
  ...
  void run()
  {
    ...
    auto caCertDatabase = getCertificateDatabase(
      formatFilename(
        FilenameTemplate.certificateDatabase, cmdline.caName, cmdline.caPath)
    );
    ...
    else
    {
      throw new ApplicationError(
        "Failed to store certificate: Name (and serial number) must be unique");
    }
  }
  ...
  void help()
  {
    ...
    writefln("--sign --template=%s --ca-name=<name-of-ca> [--ca-path=] [--path=<path-to-cert-and-csr>] <name-of-csr>", templates
        .join("|"));
  }
}
