...
class SelfSigningCertificateAuthorityCmdlinePlugin : Application
{
  ...
  private SelfSigningCertificateAuthority getCertificateAuthority(
    const string caKeyFilename, const string templateName) const
  {
  ...
  void help()
  {
    ...
    writefln("--self-sign [--path=<path-to-read-subject-and-store-cert>] --template=%s <name-of-certificate>", templates
        .join("|"));
  }
}
